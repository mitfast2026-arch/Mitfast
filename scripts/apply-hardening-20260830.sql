-- ===========================================================================
-- MITFAST PRODUCTION SECURITY HARDENING & GST MIGRATION
-- Project: qubphaacuuwlpdrsprjl (https://qubphaacuuwlpdrsprjl.supabase.co)
-- Migration: 20260830000004_production_rpc_grants_and_rfq_gst.sql
--
-- Instructions:
-- 1. Open Supabase Dashboard for project qubphaacuuwlpdrsprjl:
--    https://supabase.com/dashboard/project/qubphaacuuwlpdrsprjl/sql/new
-- 2. Paste the entire SQL below and click 'RUN'.
-- 3. Verify in terminal with: npm run test:remediation-smoke && npx tsx scripts/verify-production-security.ts
-- ===========================================================================

-- 1. GST snapshot columns on rfq_items
...
-- 2. Drop leftover legacy overloads
...
-- 3. increment_product_view: pin search_path
ALTER FUNCTION public.increment_product_view(uuid) SET search_path = public;

-- 4. Atomic RFQ RPCs with GST snapshot support
...
-- 5. api_rate_limit_log: service-role only
...
-- 6. Revoke PUBLIC execute on all non-trigger SECURITY DEFINER RPCs
DO $$
DECLARE
  r RECORD;
